...
This report is divided into two parts. In the first part we describe various applications of homographies. In Section \ref{sec:person} we map the position of a persons position in some plane onto an overview map.
In Section \ref{sec:linear} we apply linear texture mapping onto the ground plane in the atrium of ITU.

Lastly in Section \ref{sec:aug} we do augmentation of images i.e projecting 3D objects onto a 2D plane.
...
%mw
After calibrating the camera, the following data is available. The calibration matrix $K$\todo{AH! er K = calibration eller camera matrix vs P = camera eller projection matrix ??}, lists of rotation and translation vectors, one for each sample done in the calibration, the first frame of the samples and a set of points corresponding to the corners of the chess board in world coordinates.\\
...
A recorded sequence, showcasing animations, transformations and alternative objects, can be found on the enclosed DVD in a file called 'animated\_alternative\_objects.avi' \todo{lav den video og gem den ;o)}
